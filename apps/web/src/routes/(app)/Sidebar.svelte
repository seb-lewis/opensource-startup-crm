<script lang="ts">
	...
	let {
		...
		user = {},
		org_name = 'BottleCRM'
	}: { drawerHidden?: boolean; user?: any; org_name?: string } = $props();
	...
	const handleSettingsLinkClick = (event: MouseEvent, href: string) => {
	...
	const handleDropdownClick = (event: MouseEvent | KeyboardEvent) => {
		// Prevent clicks inside dropdown from bubbling up
		event.stopPropagation();
	};

	const handleClickOutside = (event: MouseEvent) => {
		if (userDropdownOpen && dropdownRef && !dropdownRef.contains(event.target as Node)) {
			userDropdownOpen = false;
		}
	};
	...
	let openDropdowns = $state({} as Record<string, boolean>);
	...
	const navigationItems = [
		...
		},
		{
			...
			type: 'dropdown',
			children: [
				{ href: '/app/leads', label: 'Open Leads', icon: List },
...
>
	<div class="flex h-full flex-col bg-white dark:bg-gray-900">
		...
		>
			...
			<button
				onclick={closeDrawer}
				class="rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 lg:hidden dark:text-gray-400 dark:hover:bg-gray-800 dark:focus:ring-gray-600"
			>
				<X class="h-5 w-5" />
			</button>
		</div>
		...
		<div class="border-t border-gray-200 p-4 dark:border-gray-700" bind:this={dropdownRef}>
			<div class="mb-3 flex items-center gap-3">
				<img class="h-10 w-10 rounded-lg object-cover" src={user.image} alt="User avatar" />
