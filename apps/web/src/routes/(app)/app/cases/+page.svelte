<script lang="ts">
	...
	import { casePriorityVisuals, caseStatusVisuals } from '$lib/data';
	import { CASE_STATUS_OPTIONS, TASK_PRIORITY_OPTIONS } from '@opensource-startup-crm/constants';
	...
	// Use options from server data (fallbacks to avoid undefined errors)
	...
	// Colors are sourced from central visuals maps for consistency
	const statusBadgeClass = (status: string) =>
		caseStatusVisuals[status as keyof typeof caseStatusVisuals]?.badgeColor ?? '';
	const priorityBadgeClass = (priority: string) =>
		casePriorityVisuals[priority as keyof typeof casePriorityVisuals]?.badgeColor ?? '';
				...
				<div class="hidden overflow-x-auto lg:block">
					<table class="w-full">
						...
						>
							<tr>
								<th
									class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400"
									>Case</th
								>
								<th
									class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400"
								...
								>
								<th
									class="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400"
									>Actions</th
								>
							</tr>
						</thead>
						<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
							{#each filteredCases as c}
								...
								>
									...
									<td class="px-6 py-4">
										<span
											class={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${priorityBadgeClass(c.priority)}`}
										>
									...
									<td class="px-6 py-4">
										<span
											class={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${statusBadgeClass(c.status)}`}
										>
					...
					{#each filteredCases as c}
						...
						>
							<div class="mb-3 flex items-start justify-between">
								...
								<span
									class={`inline-flex items-center rounded-full border px-2 py-1 text-xs font-medium ${statusBadgeClass(c.status)}`}
								>
									{toLabel(c.status, CASE_STATUS_OPTIONS)}
								</span>
							</div>
							...
							<div class="mb-4 grid grid-cols-2 gap-3 text-sm">
								...
								<div>
									...
									<span
										class={`ml-1 inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${priorityBadgeClass(c.priority)}`}
									>
										{toLabel(c.priority, TASK_PRIORITY_OPTIONS)}
									</span>
								</div>
