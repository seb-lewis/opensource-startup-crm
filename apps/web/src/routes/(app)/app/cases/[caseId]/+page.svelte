<script lang="ts">
	...
	import { caseStatusVisuals, casePriorityVisuals } from '$lib/data/index.js';
	let StatusIcon = $derived(
		data.caseItem.status === 'OPEN'
			? AlertCircle
			: data.caseItem.status === 'IN_PROGRESS'
				? RotateCcw
				: CheckCircle
	);
</script>
		...
		>
			<div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
				...
				<div class="flex gap-3">
					<a
						href="/app/cases/{data.caseItem.id}/edit"
						...
						<div class="space-y-4">
							{#if data.caseItem.dueDate}
								<div class="flex items-center gap-3">
									...
									<div>
										...
										<p class="text-gray-900 dark:text-white">{formatDate(data.caseItem.dueDate)}</p>
									</div>
								</div>
							{/if}
							...
							<div class="flex items-center gap-3">
								...
								<div>
									...
									<p class="text-gray-900 dark:text-white">{formatDate(data.caseItem.createdAt)}</p>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="space-y-4">
						<div>
							...
							<div class="flex items-center gap-2">
								...
								<span
									class="rounded-full border px-3 py-1 text-sm font-medium {caseStatusVisuals[
										data.caseItem.status as keyof typeof caseStatusVisuals
									]?.badgeColor || ''}"
								>
						...
						<div>
							...
							<span
								class="rounded-full border px-3 py-1 text-sm font-medium {casePriorityVisuals[
									data.caseItem.priority as keyof typeof casePriorityVisuals
								]?.badgeColor || ''}"
							>
