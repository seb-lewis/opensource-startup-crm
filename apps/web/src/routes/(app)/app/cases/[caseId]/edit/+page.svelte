<script lang="ts">
	...
	let accountId = $state(data.caseItem.account?.id || '');
	let dueDate = $state('');
	...
	let assignedId = $state(data.caseItem.owner?.id || '');
...
<div class="min-h-screen bg-gray-50 py-8 dark:bg-gray-900">
	<div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
		...
		{#if showCloseConfirmation}
			<div
				class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 dark:bg-black dark:bg-opacity-70"
			>
