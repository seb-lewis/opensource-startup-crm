<script lang="ts">
	...
	import { TASK_PRIORITY_OPTIONS } from '@opensource-startup-crm/constants';
	...
	import { taskPriorityVisualMap, opportunityStages } from '$lib/data';
			...
			<div class="flex flex-col py-6 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex items-center gap-4">
					{#if primaryAccountRel}
						<a
							href="/app/accounts/{primaryAccountRel.account.id}"
							...
							{#each contact.accountRelationships as relationship}
								...
								>
									<div class="flex-1">
										<div class="flex items-center gap-3">
											<a
												href="/app/accounts/{relationship.account.id}"
				...
				{#if contact.opportunities && contact.opportunities.length > 0}
					...
					>
						<div class="mb-6 flex items-center justify-between">
							...
							<a
								href="/app/opportunities?contact={contact.id}"
							...
							{#each contact.opportunities as opp}
								...
								>
									...
									<div class="text-right">
										...
										<span
											class="inline-flex rounded-full px-2 py-1 text-xs {opportunityStages.find(
												(s) => s.value === opp.stage
											)?.color || ''}"
										>
											{opp.stage.replace('_', ' ')}
								...
								<div class="flex items-start gap-3 rounded-lg bg-gray-50 p-3 dark:bg-gray-700/50">
									...
									<div class="min-w-0 flex-1">
										...
										<div class="mt-1 flex items-center gap-2">
											<span
												class="inline-flex rounded px-2 py-0.5 text-xs {taskPriorityVisualMap.find(
													(p) => p.value === task.priority
												)?.badgeColor || 'bg-gray-50 text-gray-600 dark:bg-gray-900/20'}"
											>
				...
				{#if contact.events && contact.events.length > 0}
					...
					>
						<div class="mb-6 flex items-center justify-between">
							...
							<a
								href="/app/events?contact={contact.id}"
								class="text-sm text-blue-600 hover:underline dark:text-blue-400">View all</a
							>
						</div>
						<div class="space-y-3">
