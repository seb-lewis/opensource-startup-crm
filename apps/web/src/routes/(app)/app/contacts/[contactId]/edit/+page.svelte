<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		if (res.ok) {
			await goto(`/app/contacts/${contact?.id}`, { invalidateAll: true });
		} else {
			...
			const msg =
				resp && typeof resp === 'object' && 'message' in resp
					...
					: undefined;
			errorMsg = msg || 'Failed to update contact.';
		}
		submitting = false;
	}
</script>
						...
						<div>
							...
							<div class="relative">
								...
								<input
									id="email"
									type="email"
									class="w-full rounded-lg border border-gray-300 bg-white py-3 pl-10 pr-4 text-gray-900 placeholder-gray-500 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:ring-blue-400"
									bind:value={email}
