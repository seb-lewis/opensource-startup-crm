				...
				>
					<div class="flex flex-1 flex-col gap-4 md:flex-row md:items-center">
						<div class="flex min-w-[120px] flex-col gap-1">
							<span
								class="w-fit rounded-full px-2 py-0.5 text-xs font-bold uppercase tracking-widest {quoteStatusVisuals[
									invoice.status
								]?.badgeColor || 'bg-gray-100 text-gray-700'}">{invoice.status.toLowerCase()}</span
							>
