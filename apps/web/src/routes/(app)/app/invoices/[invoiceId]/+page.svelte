<script lang="ts">
	...
	import { quoteStatusVisuals } from '$lib/data';
	import { toLabel } from '$lib/data/enum-helpers';
	import { QUOTE_STATUS_OPTIONS } from '@opensource-startup-crm/constants';
...
<div class="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 py-10">
	...
	>
		...
		<div class="mb-8 grid grid-cols-1 gap-8 md:grid-cols-2">
			...
			<div>
				<div class="mb-1 flex justify-between text-sm text-blue-600">
					<span>Status:</span>
					<span
						class="inline-block rounded-full px-3 py-1 text-xs font-semibold {quoteStatusVisuals[
							invoice.status
				...
				<div class="mb-1 flex justify-between text-sm text-blue-600">
					<span>Created:</span><span>{formatDate(invoice.createdAt)}</span>
				</div>
				<div class="flex justify-between text-sm text-blue-600">
					<span>Due Date:</span><span>{formatDate(invoice.expirationDate)}</span>
				</div>
			</div>
		</div>

		<div class="mb-8 border-b border-t border-blue-200">
			<table class="w-full text-sm">
				<thead>
					<tr class="bg-gradient-to-r from-blue-50 to-purple-50 text-xs uppercase text-blue-700">
				...
				<tbody>
					{#each invoice.lineItems as item}
						<tr class="border-b border-blue-100 hover:bg-blue-50/60">
							<td class="p-3">{item.description || item.productName || 'N/A'}</td>
		...
		<!-- Decorative elements -->
		<div
			class="absolute right-0 top-0 h-32 w-32 -translate-y-16 translate-x-16 rounded-full bg-gradient-to-bl from-purple-200/30 to-transparent"
		></div>
		<div
