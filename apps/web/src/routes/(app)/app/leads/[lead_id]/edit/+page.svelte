<script lang="ts">
	...
	import {
		...
		ratingOptions,
		industryOptions,
		leadStatusOptionsWithColor as statusOptions
	...
	function validatePhoneInput(event: Event) {
		const input = (event.target as HTMLInputElement)?.value?.trim() || '';
		if (!input) return;
		if (!isPossiblePhoneNumber(input, 'US') || !isValidPhoneNumber(input, 'US')) {
			(event.target as HTMLInputElement).setCustomValidity('Please enter a valid phone number');
		} else {
			(event.target as HTMLInputElement).setCustomValidity('');
		}
	}

	function formatPhoneBlur(event: Event) {
		...
		>
			...
			<div class="flex justify-end space-x-4 border-t border-gray-200 pt-6 dark:border-gray-700">
				<button
					type="button"
					onclick={cancelEdit}
					class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-700 transition-all hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 dark:focus:ring-blue-400 dark:focus:ring-offset-gray-800"
				>
				...
				<button
					...
					class="inline-flex items-center rounded-lg border border-transparent bg-blue-600 px-6 py-3 text-sm font-medium text-white transition-all hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-400 dark:focus:ring-offset-gray-800"
				>
					{#if isSubmitting}
