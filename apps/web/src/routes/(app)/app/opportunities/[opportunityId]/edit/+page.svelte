<script lang="ts">
	...
	let closeDateStr = $state(
		opportunity.closeDate ? new Date(opportunity.closeDate).toISOString().slice(0, 10) : ''
	);
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		if (res.ok) {
			...
			const result = (await res.json()) as { message?: string } | undefined;
	...
	import {
		opportunityStages,
		sourceOptions as leadSources,
		forecastCategoryOptions as forecastCategories
	} from '$lib/data';
