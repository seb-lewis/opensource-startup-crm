<script lang="ts">
	...
	import { TASK_STATUS_OPTIONS, TASK_PRIORITY_OPTIONS } from '@opensource-startup-crm/constants';
	...
	// Local task state kept in sync with optimistikit data
	let task = $state(data.task);

	// Optimistikit setup
	const { enhance: optimisticEnhance, data: optimistic_data } = optimistikit(() => data);
	$effect(() => {
		if (optimistic_data?.task) task = optimistic_data.task;
	});
	// Comments are now part of the task object from the server

	let newComment = $state('');

	// Optimistic add comment handler (mutates optimistic data)
	function onOptimisticTaskAddComment(optimistic: any, { formData }: { formData: FormData }) {
		const body = formData?.get('commentBody');
		const text = typeof body === 'string' ? body : body?.toString?.();
		const trimmed = (text || '').trim();
		if (!trimmed) return;
		const current = optimistic.task?.comments || [];
		optimistic.task = {
			...optimistic.task,
			comments: [
				...current,
				{
					...
					author: {
						...
						name: data?.loggedInUser?.name || 'You',
						image: data?.loggedInUser?.image || null
					}
				}
			]
		};
			...
			>
				...
				<div class="border-b border-gray-100 p-4 dark:border-gray-700">
					<div class="mb-3 flex flex-wrap items-center gap-2">
						{#if statusItem}
							<span
								class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium {statusItem.badgeColor}"
							>
								{statusItem.label}
							</span>
						{:else}
						...
						{#if priorityItem}
							<span
								class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium {priorityItem.badgeColor}"
							>
								{priorityItem.label}
							</span>
						{:else}
						...
						<div class="ml-auto flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
							...
							<span>Due {formatDate(task.dueDate, 'en-US', undefined, '-')}</span>
						</div>
					</div>
					...
					{:else}
						<p class="text-sm italic text-gray-500 dark:text-gray-400">No description provided</p>
					{/if}
				</div>
			...
			>
				...
				<div class="p-4">
					...
					<form
						method="POST"
						action="?/addComment"
						use:optimisticEnhance={onOptimisticTaskAddComment}
						class="space-y-3"
					>
						<div>
							<label
							...
							<textarea
								id="commentBody"
								name="commentBody"
								class="w-full resize-none rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-500 transition-colors focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:ring-blue-400"
								rows="2"
