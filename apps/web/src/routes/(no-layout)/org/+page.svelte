<script lang="ts">
	 ...
	 */
	function selectOrg(org: { id: string; name: string }) {
...
<div class="min-h-screen bg-gray-50 p-4">
	<div class="mx-auto max-w-6xl">
		...
		<!-- Organizations Grid -->
		{#if orgs.length > 0}
			<form id="org-form" method="POST" action="?/select" class="hidden">
				<input id="orgId" name="orgId" type="hidden" />
			</form>
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
				{#each orgs as org}
					<button
						class="group w-full cursor-pointer rounded-xl border border-gray-200 bg-white text-left shadow-sm transition-all duration-200 hover:border-blue-300 hover:shadow-md"
					...
					>
						<div class="p-6">
							...
							<div
								class="w-full rounded-lg bg-blue-600 px-4 py-2.5 font-medium text-white transition-colors duration-200 group-hover:bg-blue-700"
							>
								Select Organization
							</div>
						</div>
		...
		{#if orgs.length > 0}
			<a
				href="/org/new"
				class="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-blue-600 text-white shadow-lg transition-all duration-200 hover:bg-blue-700 hover:shadow-xl"
